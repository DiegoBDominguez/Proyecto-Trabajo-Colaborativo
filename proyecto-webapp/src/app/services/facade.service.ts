...
// Funciones básicas
  ...
  getSessionToken(){
    // Prefer cookie token if present (legacy), otherwise fallback to localStorage (used by AuthService)
    const cookieToken = this.cookieService.get(session_cookie_name);
